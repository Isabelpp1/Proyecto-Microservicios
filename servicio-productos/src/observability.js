...
function correlationMiddleware(service) {
  return (req, res, next) => {
    ...
    logEvent(service, 'info', 'request_received', { correlationId, method: req.method, path: req.originalUrl });
    res.on('finish', () => logEvent(service, 'info', 'request_completed', {
      correlationId, method: req.method, path: req.originalUrl, status_code: res.statusCode,
